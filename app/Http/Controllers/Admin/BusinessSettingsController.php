...
// dd( $request['commission_business_model']);
    ...
    {
        $json = BusinessSetting::where(['key' => 'truceptWG'])->value('value');
        if (empty($json)) {
            $truceptWG = [
                'status' => 0,
            ...
            $truceptWG = json_decode($json, true);
        }
        $fcm_credentials = Helpers::get_business_settings('fcm_credentials');
        return view('admin-views.business-settings.fcm-config', compact('fcm_credentials', 'truceptWG'));
    }
    ...
    public function update_fcm(Request $request)
    {
        ...
        Helpers::businessUpdateOrInsert(['key' => 'truceptWG'], [
            'value' => json_encode([
                'status' => $request['truceptWG_status'],
                'nodeurl' => $request['truceptWG_nodeurl'],
                'access_token' => $request['truceptWG_token'],
                'instance_id' => $request['truceptWG_instanceid'],
                'otp_template' => $request['truceptWG_otp_template'],
            ]),
        ]);
