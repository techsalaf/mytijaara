...
//        }
    ...
    public static function send_wa_notif_to_device($receiver, $msgdata, $truceptWG)
    {
        ...
        $queryString = http_build_query([
            ...
            'instance_id'  => $truceptWG['instance_id'],
            'access_token' => $truceptWG['access_token'],
        ]);
    

        $url = 'https://wa.trucept.pro/api/send?' . $queryString;
        ...
        return json_encode(['success' => false, 'error' => $responseArray['message'] ?? 'Unknown error']);
    }
    ...
    public static function send_order_notification($order)
    {
        ...
        try {
	    $truceptWG = self::get_settings('truceptWG');
		...
		$user_phone = $customer_details['contact_person_number'];
		...
		$user_phone = $order?->customer?->phone;
            ...
            if (self::getNotificationStatusData('customer','customer_order_notification','push_notification_status') &&  $value && $user_fcm) {
                ...
                if (isset($truceptWG) && $truceptWG['status'] == 1) {
                    self::send_wa_notif_to_device($user_phone, $data, $truceptWG);
                    if ($status == 'picked_up' && \Config::get('order_delivery_verification') == 1) {
                        $data['description'] = str_replace("#OTP#", $order->otp, $truceptWG['otp_template']) . ' (delivery verification)';
                        self::send_wa_notif_to_device($user_phone, $data, $truceptWG);
                    }
                }
            ...
            if ($status == 'picked_up') {
	            ...
	            if (isset($truceptWG) && $truceptWG['status'] == 1) self::send_wa_notif_to_device($order->store->phone, $data, $truceptWG);
                    DB::table('user_notifications')->insert([
                        'data' => json_encode($data),
		...
		if (isset($truceptWG) && $truceptWG['status'] == 1) self::send_wa_notif_to_device($order->delivery_man->phone, $data, $truceptWG);
                DB::table('user_notifications')->insert([
                    'data' => json_encode($data),
