...
@section('content')
                ...
                <div class="card">
                    <div class="card-header py-1 border-0">
                        <div class="search--button-wrapper justify-content-end flex-wrap">
                            <h4 class="flex-grow-1 mb-0">{{ $allStoreReelsListLabel }}</h4>
                            <form class="search-form min--260" action="{{ route('admin.reels.index') }}" method="GET">
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center flex-nowrap min-w-220 overflow-hidden" title="{{ $reel->store?->name }}">
                                                ...
                                                <div class="ml-3">
                                                    ...
                                                    @else
                                                        <h5 class="text-wrap line--limit-1 min-w-160 max-w-200px mb-0">
                                                            {{ $deletedStoreLabel }}
                                                        </h5>
                                                    @endif
                                                </div>
    ...
    <div id="datatableFilterSidebar" class="hs-unfold-content_ sidebar sidebar-bordered sidebar-box-shadow initial-hidden w-500px max-w-100">
        <div class="card card-lg sidebar-card sidebar-footer-fixed">
            ...
            <form class="card-body sidebar-body sidebar-scrollbar" action="{{ route('admin.reels.index') }}" method="GET" id="reels_filter_form">
...
@push('script_2')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            const reelStatusSelector = '#reels_filter_form input[name="reel_status[]"]';
            const allReelStatusSelector = '#reels_filter_form input[name="reel_status[]"][value="all"]';

            function syncReelStatusAllCheckbox() {
                const $statusCheckboxes = $(reelStatusSelector).not('[value="all"]');
                const $allCheckbox = $(allReelStatusSelector);
                const allSelected = $statusCheckboxes.length > 0 && $statusCheckboxes.length === $statusCheckboxes.filter(':checked').length;

                $allCheckbox.prop('checked', allSelected);
            }

            $(document).on('change', reelStatusSelector, function () {
                ...
                if (value === 'all') {
                    $statusCheckboxes.prop('checked', isChecked);
                } else if (!isChecked) {
                    $allCheckbox.prop('checked', false);
                } else {
                    syncReelStatusAllCheckbox();
                }
            });
                            ...
                            labels: {
                                ...
                                total: {
                                    ...
                                    formatter: function(w) {
                                        ...
                                        return formatCompactValue(total);
                                    }
                                }
                            }
                        }
