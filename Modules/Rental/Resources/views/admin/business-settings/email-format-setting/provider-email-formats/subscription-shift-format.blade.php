    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','rental_subscription_shift_mail_status_provider')->first()?->value ?? '0')
                    ...
                    <div class="card-body">
                        ...
                        <span class="form-label-secondary text--primary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('If_a_Provider_successfully_shift_to_a_plan_they_will_get_a_confirmation_email.') }}">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                ...
                                <input type="checkbox" class="status toggle-switch-input dynamic-checkbox"
                                       ...
                                       data-text-on="<p>{{translate('If_enabled,_providers_will_get_a_subscription_shift_email_when_they_successfully_shift_their_subscription.')}}</p>"
                                       data-text-off="<p>{{translate('If_disabled,_providers_will_not_get_a_subscription_shift_email_when_a_provider_successfully_shift_their_subscription.')}}</p>"
                        ...
                        <form action="{{route('admin.business-settings.rental-email-status',['provider','subscription-shift',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                @php($data=\Modules\Rental\Entities\RentalEmailTemplate::where('type','provider')->where('email_type', 'subscription-shift')->first())
                @php($template= $template ?? $data?->email_template ?? 5)
                <form action="{{ route('admin.business-settings.rental-email-setup', ['provider','subscription-shift']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\Modules\Rental\Entities\RentalEmailTemplate::withoutGlobalScope('translate')->where('type','provider')->where('email_type', 'subscription-shift')->first())
