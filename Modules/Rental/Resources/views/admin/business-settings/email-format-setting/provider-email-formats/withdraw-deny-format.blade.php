    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','rental_withdraw_deny_mail_status_provider')->first()?->value ?? '0')
                    ...
                    <div class="card-body">
                        ...
                        <span class="form-label-secondary text--primary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('If_a_Provider_requests_for_a_withdrawal_and_Admin_rejects_it,_the_Provider_will_get_an_automated_Withdraw_Rejection_email_from_the_system.') }}">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                <input type="checkbox" class="status toggle-switch-input dynamic-checkbox"
                                       ...
                                       data-text-on="<p>{{translate('If_enabled,_Providers_will_receive_an_automated_mail_from_the_system_when_the_Admin_Rejects_their_Withdraw_Request.')}}</p>"
                                       data-text-off="<p>{{translate('If_disabled,_Providers_will_not_receive_any_Withdrawal_Rejection_mail.')}}</p>"
                        ...
                        <form action="{{route('admin.business-settings.rental-email-status',['provider','withdraw-deny',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                @php($data=\Modules\Rental\Entities\RentalEmailTemplate::where('type','provider')->where('email_type', 'withdraw_deny')->first())
                @php($template= $template ?? $data?->email_template ?? 6)
                <form action="{{ route('admin.business-settings.rental-email-setup', ['provider','withdraw-deny']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\Modules\Rental\Entities\RentalEmailTemplate::withoutGlobalScope('translate')->where('type','provider')->where('email_type', 'withdraw_deny')->first())
