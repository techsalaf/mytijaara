...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="page-header">
            ...
            @include('rental::admin.business-settings.email-format-setting.partials.admin-email-template-setting-links')
        </div>

        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','rental_provider_registration_mail_status_admin')->first()?->value ?? '0')
                    ...
                    <div class="card-body">
                        <div class="maintenancemode-toggle-bar d-flex flex-wrap justify-content-between border rounded align-items-center p-2">
                            <h5 class="text-capitalize m-0 text--primary pl-2">
                                {{translate('Receive_Mail_On_‘New_Provider_Registration’?')}}
                        <span class="form-label-secondary text--primary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('If_a_provider_registers_from_the_customer_website_or_app_or_provider_app,_admin_will_receive_an_automated_email.') }}">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                <input type="checkbox" class="status toggle-switch-input dynamic-checkbox"
                                       ...
                                       data-text-on="<p>{{translate('If_enabled,_the_admin_will_get_an_automated_email_when_a_provider_registers.')}}</p>"
                                       data-text-off="<p>{{translate('If_disabled,_the_admin_will_not_get_an_automated_email_when_a_provider_registers.')}}</p>"
                        ...
                        <form action="{{route('admin.business-settings.rental-email-status',['admin','provider-registration',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                @php($data=\Modules\Rental\Entities\RentalEmailTemplate::where('type','admin')->where('email_type', 'provider_registration')->first())
                @php($template= $template ?? $data?->email_template ?? 1)
                <form action="{{ route('admin.business-settings.rental-email-setup', ['admin','provider-registration']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\Modules\Rental\Entities\RentalEmailTemplate::withoutGlobalScope('translate')->where('type','admin')->where('email_type', 'provider_registration')->first())
