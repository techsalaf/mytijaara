    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','rental_refund_order_mail_status_user')->first()?->value ??  '0' )
                    <div class="card-body">
                        ...
                        <form action="{{route('admin.business-settings.rental-email-status',['user','refund-order',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                                            </form>
                    </div>
                </div>
                @php($data=\Modules\Rental\Entities\RentalEmailTemplate::where('type','user')->where('email_type', 'refund_order')->first())
                @php($template= $template ?? $data?->email_template ?? 9)
                <form action="{{ route('admin.business-settings.rental-email-setup', ['user','refund-order']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\Modules\Rental\Entities\RentalEmailTemplate::withoutGlobalScope('translate')->where('type','user')->where('email_type', 'refund_order')->first())
